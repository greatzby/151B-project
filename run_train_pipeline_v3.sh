#!/bin/bash
# v3 版：从拒绝采样到 base/sft/rl 在 val 上的评测
# 所有产物输出到 *_v3 目录，旧目录数据保持不动
#
# 资源分配：
...
# 0 = 已有 splits_v3/train.jsonl、splits_v3/val.jsonl、sft_data_v3/sft_train.jsonl 则跳过
...
mkdir -p splits_v3 sft_data_v3 ckpts_v3 results_v3 logs
...
if [ ! -f "$PUBLIC_PATH" ]; then
    ...
    echo "请确认 data/public_v3.jsonl 存在，并且 config.py 路径已切到 v3。"
    exit 1
fi
...
echo ""
echo "================================================"
echo "[1/6] 拒绝采样：生成 SFT 训练数据（v3）"
echo "================================================"

if [ "$REBUILD_SFT_DATA" = "0" ] \
   && [ -f "splits_v3/train.jsonl" ] \
   && [ -f "splits_v3/val.jsonl" ] \
   && [ -f "sft_data_v3/sft_train.jsonl" ]; then

    echo "⏭️  跳过 prepare_sft_data.py：已有以下文件："
    echo "    splits_v3/train.jsonl"
    echo "    splits_v3/val.jsonl"
    echo "    sft_data_v3/sft_train.jsonl"
    echo ""
    echo "如需重新生成，请使用："
...
check_file_exists "splits_v3/train.jsonl"
check_file_exists "splits_v3/val.jsonl"
check_file_exists "sft_data_v3/sft_train.jsonl"

N_TRAIN=$(wc -l < splits_v3/train.jsonl)
N_VAL=$(wc -l < splits_v3/val.jsonl)
N_SFT=$(wc -l < sft_data_v3/sft_train.jsonl)
...
run_eval "Qwen/Qwen3-4B-Thinking-2507" \
         "splits_v3/val.jsonl" \
         "results_v3/eval_base_val.jsonl"
...
if [ -d "ckpts_v3/sft_merged" ] && [ -n "$(ls -A ckpts_v3/sft_merged 2>/dev/null)" ]; then
    echo "⏭️  跳过：ckpts_v3/sft_merged 已存在。"
    echo "如需重新 SFT 训练，请先删除："
    echo "    rm -rf ckpts_v3/sft ckpts_v3/sft_merged"
else
    CUDA_VISIBLE_DEVICES=0 python sft_train.py
fi

check_dir_nonempty "ckpts_v3/sft_merged"
...
run_eval "ckpts_v3/sft_merged" \
         "splits_v3/val.jsonl" \
         "results_v3/eval_sft_val.jsonl"


echo ""
echo "🎉 训练与 val 评测流程完成（v3）！"
echo ""
echo "生成的主要文件："
echo "  results_v3/eval_base_val.jsonl"
echo "  results_v3/eval_sft_val.jsonl"
echo ""