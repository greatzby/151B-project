#!/bin/bash
# v2 版 RL：pass rate 过滤 + GRPO + val 评测
#
# 前置：
#   ckpts_v2/sft_merged 已存在
#   splits_v2/train.jsonl 已存在
#   splits_v2/val.jsonl   已存在
#
# 资源：
...
FILTERED_DATA="splits_v2/train_filtered.jsonl"
...
if [ ! -d "ckpts_v2/sft_merged" ] || [ -z "$(ls -A ckpts_v2/sft_merged 2>/dev/null)" ]; then
    echo "❌ 缺少 ckpts_v2/sft_merged"
    exit 1
fi
echo "✅ ckpts_v2/sft_merged 存在"

if [ ! -f "splits_v2/train.jsonl" ]; then
    echo "❌ 缺少 splits_v2/train.jsonl"
    exit 1
fi
N_TRAIN=$(wc -l < splits_v2/train.jsonl)
echo "✅ splits_v2/train.jsonl: $N_TRAIN 条"

if [ ! -f "splits_v2/val.jsonl" ]; then
    echo "❌ 缺少 splits_v2/val.jsonl"
    exit 1
fi
N_VAL=$(wc -l < splits_v2/val.jsonl)
echo "✅ splits_v2/val.jsonl:   $N_VAL 条"
...
if [ "$RECOMPUTE_PASS_RATE" = "0" ] && [ -f "$FILTERED_DATA" ] && [ -s "$FILTERED_DATA" ]; then
    ...
    CUDA_VISIBLE_DEVICES=0,1 python compute_pass_rate.py \
        --model ckpts_v2/sft_merged \
        --input splits_v2/train.jsonl \
        --output "$FILTERED_DATA" \
        --meta_output splits_v2/pass_rate.jsonl \
...
if [ -d "ckpts_v2/rl_merged" ] && [ -n "$(ls -A ckpts_v2/rl_merged 2>/dev/null)" ]; then
    ...
    echo "   如需重训: rm -rf ckpts_v2/rl ckpts_v2/rl_merged"
else
    cleanup_vllm
...
if [ ! -d "ckpts_v2/rl_merged" ] || [ -z "$(ls -A ckpts_v2/rl_merged 2>/dev/null)" ]; then
    echo "❌ ckpts_v2/rl_merged 不存在"
    exit 1
fi
...
echo ""
echo "🎉 完成！"
echo "结果：results_v2/eval_rl_val.jsonl"
echo "对比：results_v2/eval_sft_val.jsonl（baseline）"