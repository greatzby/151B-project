...
run_voted "Qwen/Qwen3-4B-Thinking-2507" \
          "results_v2/eval_base_private_voted_long_v2.jsonl"
...
run_voted "ckpts_v2/sft_merged" \
          "results_v2/eval_sft_private_voted_long_v2.jsonl"
...
run_voted "ckpts_v2/rl_merged" \
          "results_v2/eval_rl_private_voted_long_v2.jsonl"
...
for variant in base sft rl; do
    csv_out="results_v2/submission_${variant}_voted_long_v2.csv"
    jsonl_in="results_v2/eval_${variant}_private_voted_long_v2.jsonl"
    if [ -f "$csv_out" ]; then
...
echo "🎉 全部完成！"
echo ""
echo "============ v2 候选 CSV（新 judger + ckpts_v2 模型）============"
ls -lh results_v2/submission_*.csv 2>/dev/null || echo "（暂无）"
echo ""
echo "============ 旧版候选 CSV（保留，results/）============"
ls -lh results/submission_*.csv 2>/dev/null || echo "（暂无）"
echo ""
echo "→ 旧 voted (results/):"
echo "    submission_base_voted_long.csv"
echo "    submission_sft_voted_long.csv"
echo "    submission_rl_voted_long.csv"
echo "→ 新 voted (results_v2/, 用新 judger + ckpts_v2):"
echo "    submission_base_voted_long_v2.csv"
echo "    submission_sft_voted_long_v2.csv"
echo "    submission_rl_voted_long_v2.csv"
echo ""
